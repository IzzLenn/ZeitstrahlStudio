...
Die WPF-Ansicht übermittelt beim Ende einer Kartenbewegung ausschließlich den visuellen Bewegungsunterschied, Ereignis-ID und aktuelle Orientierung an das ViewModel. Entlang der gezoomten Zeitachse wird der Viewportunterschied zuvor auf kanonische 100-%-Layoutkoordinaten normalisiert; quer zur Zeitachse bleibt er ein unskalierter DIP-Wert. Die Application-Schicht addiert diese orientierungsabhängigen Werte auf die bestehende `LayoutPosition`, begrenzt beide gespeicherten Koordinaten auf ±100.000 kanonische Einheiten und zeichnet einen Layout-Änderungsschritt in derselben maximal 100 Einträge umfassenden Undo-/Redo-Historie auf. Beim Zeichnen wird nur der achsenparallele Versatz mit dem Zoom skaliert; die Querbasis manuell positionierter Karten und die sichtbare Querachsenlage bleiben bei einem Zoomwechsel stabil. Bereits gespeicherte Zahlenwerte bleiben unverändert und erscheinen beim nicht persistenten Startzoom von 100 Prozent wie zuvor; ein Erstellungszoom war im Projektformat nie vorhanden und kann daher nicht verlustfrei migriert werden. Löschen erfasst zugehörige Positionen mit, damit Undo das vollständige sichtbare Ereignis wiederherstellt. „Auto-Layout“ ist ein einzelner gemeinsamer Historieneintrag für alle entfernten Positionen.
...
**Status:** angenommen am 19.07.2026, erweitert am 13.08.2026
...
Die Darstellung wird als semantische, responsive Vollfenster-Shell aufgebaut: Momentaufnahmehinweis, Projektkopf und Kennzahlen bleiben vom eigentlichen Arbeitsbereich getrennt; darunter liegen gruppierte Bedienung, Volltextsuche, ein platzsparend aufklappbares Filterpanel und ein horizontaler beziehungsweise vertikaler Zeitstrahl mit alternierenden Karten. Die Ereignisfarbe zeichnet den vollständigen Kartenrahmen, Balken und Marker; eine neutrale Außenkontur hält die Karte auch bei extrem hellen oder dunklen gültigen `#RRGGBB`-Werten sichtbar. In der vertikalen Desktopansicht wird die Achse relativ zum tatsächlich sichtbaren Arbeitsbereich zentriert; die einspaltige Mobilansicht behält ihre links geführte Achse. Auf schmalen Ansichten werden Kennzahlen, Werkzeuggruppen und vertikale Karten ohne Funktionsverlust umgebrochen.

Hell und Dunkel werden ausschließlich über CSS-Variablen abgebildet. Der Startwert stammt aus der lokal gespeicherten Darstellungspräferenz oder ersatzweise aus `prefers-color-scheme`; gespeichert wird nur `zeitstrahl-studio-export-theme`, niemals ein Projektinhalt. ARIA-Gruppen, `aria-pressed`, ein Live-Ergebnisstatus, sichtbare Fokuszustände, reduzierte Bewegung und die Tastenkürzel `/` beziehungsweise `Esc` ergänzen Maus-, Touch- und Tastaturbedienung. Suche, Filter und Orientierungswechsel erhalten geöffnete passende Ereignisdetails.

Der Druck ist eine reversible Darstellungstransaktion: `beforeprint` merkt Orientierung, Zoom, Scrollposition, Filterpanel, Projektbeschreibung und geöffnete Ereignisdetails, wechselt auf eine papierkontrastreiche vertikale 100-%-Ansicht und öffnet alle Inhalte. `afterprint` stellt den vorherigen Zustand nach der Layoutberechnung wieder her. Die Content Security Policy sperrt zusätzlich `connect-src`, `font-src`, `media-src`, `object-src`, `frame-src`, `worker-src`, `manifest-src`, `base-uri` und `form-action`; Nutzdaten gelangen weiterhin ausschließlich über sichere Textknoten und DOM-Attribute in die Seite, niemals über `innerHTML`.
...
**Status:** angenommen am 27.07.2026, erweitert am 13.08.2026
...
Das obere Hauptmenü verwendet ein ausschließlich in `MainMenu.Resources` definiertes `MenuItem`-Template. Die visuelle Windows-Abnahme zeigte, dass das native WPF-Template den hervorgehobenen Eintrag trotz gesetzter semantischer Brushes mit einer hellen Systemfläche zeichnete und dadurch die helle Darkmode-Beschriftung unlesbar machte. Das lokale Template zeichnet Popup, Hervorhebung, geöffneten Zustand, Tastaturfokus und angehakte Einträge mit den vorhandenen semantischen Menü- und Auswahlressourcen. Header, Befehle, Eingabegesten, Untermenüs, Häkchen und Tastaturnavigation bleiben beim nativen `MenuItem`; Kontextmenüs und andere Auswahlsteuerelemente übernehmen das Template bewusst nicht.

Kontrollkästchen verwenden ein gemeinsames WPF-`CheckBox`-Template, weil das native Standardtemplate Haken und Teilmarkierung im Dunkelmodus erst auf einer unerwünscht hellen Hoverfläche sichtbar machte. Ungeprüft, geprüft, unbestimmt und deaktiviert erhalten dauerhaft kontrastierte semantische Flächen und Glyphen; Hover und Tastaturfokus ändern ausschließlich Rahmen beziehungsweise Fokusdicke. `IsChecked`, Drei-Zustands-Verhalten, `ContentSource`, Tabstopp, Leertaste, Two-Way-Binding und UI-Automation verbleiben beim nativen `CheckBox`-Steuerelement.

Der Theme-Dienst synchronisiert zusätzlich die native Titelleiste aller bereits erzeugten anwendungseigenen WPF-Fenster; ein globaler `Window.Loaded`-Handler erfasst später geöffnete Dialoge. Die best-effort DWM-Anbindung verwendet `DWMWA_USE_IMMERSIVE_DARK_MODE` mit dem Legacy-Fallback und setzt auf Windows 11 zusätzlich Beschriftungs- und Textfarbe aus den semantischen Theme-Ressourcen. Beim Hellmodus werden diese Farben auf die Windows-Standardwerte zurückgesetzt. Fehlende DWM-Funktionen dürfen weder Anwendungsstart noch Dialogöffnung verhindern; eine eigene Fensterchromleiste wurde verworfen, weil sie native Verschiebe-, Größen-, DPI- und Barrierefreiheitssemantik unnötig duplizieren würde.
## ADR-039: Ein zentral geprüfter Dokumentpfad für Doppelklick und Projekttransfer

**Status:** angenommen am 13.08.2026

Jeder Dateianhang wird weiterhin beim Import vollständig, streamend und unter einer GUID-basierten internen Adresse in den Projektarbeitsordner kopiert. Eine zweite Speicher- oder Öffnungsimplementierung für den Doppelklick wurde verworfen. Der sichtbare Anhangseintrag übergibt stattdessen seine ID an das Haupt-ViewModel; dort wird der tatsächliche Anhang erneut aus dem aktuell ausgewählten Ereignis aufgelöst und ausschließlich über `IAttachmentFileService` geöffnet. Dieser Dienst begrenzt den Pfad auf den Workspace, sperrt Reparse Points und prüft Stabilität, Länge sowie SHA-256, bevor Windows die Projektkopie mit `UseShellExecute` an das konfigurierte Standardprogramm übergibt.

Ein Doppelklick ist eine besonders kurze Startaktion. Deshalb blockiert dieser Weg bekannte ausführbare, Skript-, Installer- und Verknüpfungsendungen. Die vorhandene mehrstufige Schaltfläche **Öffnen** bleibt für einen ausdrücklich gewählten Start verfügbar. Ein Positivlistenmodell wurde verworfen, weil es legitime lokale Fachformate unnötig ausschließen würde; die zentrale Integritätsprüfung bleibt für beide Wege verbindlich.

Der Projektarchivdienst vergleicht die beim tatsächlichen ZIP-Schreiben ermittelten Werte jeder in SQLite referenzierten Dokumentkopie mit deren gespeichertem Pfad, Größe und SHA-256. Erst danach darf das verifizierte temporäre Archiv das Ziel atomar ersetzen. Damit wird der schon bestehende Kopier- und Transfervertrag nicht dupliziert, sondern an seiner letzten Ausgabegrenze erzwungen. Das `.zeitprojekt`-Format und sein Formatversionswert bleiben unverändert.

## ADR-040: Optionales atomar transportierbares HTML-Dokumentpaket

**Status:** angenommen am 13.08.2026

Der HTML-Export behält ohne Dokumentoption die vollständig kompatible `.html`-/`.htm`-Einzeldatei. Bei aktivierter Dokumentoption erzeugt derselbe Application-Port stattdessen ein einzelnes ZIP-Artefakt. Ein direkt beschriebener Zielordner wurde verworfen, weil ein Verbund mehrerer lose geschriebener Dateien weder als Ganzes atomar ersetzt noch so zuverlässig weitergegeben werden kann. Das Paket enthält `index.html`, `LESMICH.txt` und validierte Kopien unter `Dokumente/{Anhang-ID}.{sichere Endung}`. Originaldateinamen bleiben sichtbare Nutzdaten, werden aber nie als Archivpfade verwendet; dadurch bleiben gleichnamige Dateien kollisionsfrei und Pfadtraversal sowie reservierte Windows-Namen ausgeschlossen.

Vor jeder Übernahme fordert der Export den lokalen Pfad über `IAttachmentFileService` an. Während des Kopierens werden Länge und SHA-256 nochmals bestimmt, das geschlossene ZIP wird anschließend einschließlich aller Dokumenteinträge erneut geprüft und erst dann atomar übernommen. Fehlt eine Kopie oder weicht ihre Integrität ab, bleibt ein vorhandener Export unverändert. Es wurde keine neue Produktionsabhängigkeit eingeführt; `System.IO.Compression` und `System.Security.Cryptography` aus .NET 8 genügen.

Der Payload enthält relative, ausschließlich vom Programm erzeugte Dokumentpfade. Dokumentnamen werden weiterhin nur als Textknoten gesetzt; Namen und vorhandene Miniaturen werden im Paket zu lokalen Links mit `noopener`, `noreferrer` und ohne Referrer. Die restriktive Content Security Policy bleibt bestehen und es werden keine Netzwerk- oder Shell-APIs in die HTML-Datei aufgenommen. Ein Browser kann lokale Bilder oder PDFs direkt anzeigen, andere Formate aber abhängig von seinen Sicherheitseinstellungen nur herunterladen oder an Windows weiterreichen; ein zuverlässiger direkter `ShellExecute` aus einer lokalen HTML-Datei wäre nicht sicher und wurde deshalb nicht nachgebildet.

Der orangefarbene Momentaufnahmehinweis bleibt aus Rückwärtskompatibilitäts- und Transparenzgründen standardmäßig aktiv. Ein unabhängiges Exportflag darf ihn für die konkrete Momentaufnahme ausblenden. Das Element startet im HTML mit `hidden`, bevor das eingebettete Payload ausgewertet wird, sodass bei deaktivierter Option kein kurzes Aufblitzen entsteht.
